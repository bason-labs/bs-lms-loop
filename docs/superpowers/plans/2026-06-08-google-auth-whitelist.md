...
In Firestore, create collection `admins` with a document whose **ID is your admin Google email** (e.g. `you@example.com`), fields: `{ role: "admin" }`. Leave `whitelist` empty for now (the admin page will fill it).
