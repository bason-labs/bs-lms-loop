...
(function () {
  ...
  const log = (...a) => { try { console.log('%c[LMS Loop]', 'color:#40e0d0;font-weight:700', ...a); } catch { /* ignore */ } };

  function isDisabled(el) {
    if (!el) return true;
    const cls = el.className && el.className.toString ? el.className.toString() : '';
    return !!(el.disabled || el.getAttribute('aria-disabled') === 'true' || /\b(disabled|is-disabled)\b/.test(cls));
  }
  ...
  NS.dom = {
    ...
    simulateClick, setNativeValue, clickVisible, typeVisible, isDisabled, log
  };
  NS.log = log;
})();
